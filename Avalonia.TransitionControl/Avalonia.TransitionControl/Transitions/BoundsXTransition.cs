...
using Avalonia.TransitionControl.Controls;
...
/// </summary>
public class BoundsXTransition : IPageTransition
{
    /// <summary>
    ...
    public BoundsXTransition(TimeSpan duration, SlideAxis orientation = SlideAxis.Horizontal)
    {
        Duration = duration;
    ...
    public virtual async Task Start(Visual from, Visual to, bool forward, CancellationToken cancellationToken)
    {
        ...
        DirectProperty<AnimatableUserControl, double> boundsProperty = Orientation == SlideAxis.Horizontal ? AnimatableUserControl.BoundsXProperty : AnimatableUserControl.BoundsYProperty;
        ...
        if (from != null)
        {
            var animation = new Animation.Animation
            {
                ...
                Children =
                {
                    new KeyFrame
                    {
                        Setters = { new Setter { Property = boundsProperty, Value = 0d } },
                        Cue = new Cue(0d)
                    },
                    new KeyFrame
                    {
                        Setters = { new Setter { Property = boundsProperty, Value = forward ? -distance : distance } },
                        Cue = new Cue(1d)
                    }
                },
                Duration = Duration
